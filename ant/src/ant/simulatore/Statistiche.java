...
public class Statistiche {
	...
	public Map<Object, Integer> raccoltoPerFormica(Set<Cibo> raccolto) {
		...
		final Map<Object,Integer> mezzo2quantita = new HashMap<>();
		for(Cibo c : raccolto) {
			Formica raccoglitore = c.getRaccoglitrice();
...
//				vecchiaQuantita++;
	...
	public Map<Class<?>, Set<Cibo>> raccoltoPerTipoDiFormica(Set<Cibo> raccolto) {
		...
		final Map<Class<?>,Set<Cibo>> tipo2quantita = new HashMap<>();
		for(Cibo c : raccolto) {
			Formica racc = c.getRaccoglitrice();
			Class<?> cl = racc.getClass();
			if(!tipo2quantita.containsKey(cl)) {
				Set<Cibo> numeroCibo = new HashSet<>();
				numeroCibo.add(c);
				tipo2quantita.put(cl, numeroCibo);
			}
			else {
				Set<Cibo> cibo = tipo2quantita.get(cl);
				cibo.add(c);
				tipo2quantita.put(cl, cibo);
			}
		}
		return tipo2quantita;
	}
